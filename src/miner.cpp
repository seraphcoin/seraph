...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("seraphMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("seraphMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("seraphMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("seraph-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running seraphMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
